...
export default function TechnicalDocumentsPage() {
  ...
  return (
    <AppShell
      ...
      subtitle="Engineering drawings, specs sheets & technical reports."
      activeRole={role}
            ...
            {defaultDocuments.map((doc) => (
              <tr key={doc.id} className="hover:bg-graphite-50/50">
                <td className="px-5 py-3.5 flex items-center gap-2 font-medium text-graphite-800">
                  <FileText size={16} className="text-teal-600" /> {doc.name}
                </td>
                ...
                <td className="px-5 py-3.5 text-right">
                  <button className="inline-flex items-center gap-1 rounded-md border border-graphite-200 px-2.5 py-1 text-xs font-semibold text-graphite-700 hover:border-teal-300 hover:text-teal-700">
                    <Download size={13} /> Download Specs
                  </button>
                </td>
              </tr>
            ))}
